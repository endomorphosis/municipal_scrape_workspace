...
# Estimated: 22 collections, up to ~6600 index files, ~20-25 TB total
...
download_one() {
  ...
  while [ "$attempt" -le "$RETRIES" ]; do
    if have_cmd curl; then
      # Note: we capture http_code via -w; curl may still fail to connect, so default code=000
      local code
      ...
      rm -f "$part" 2>/dev/null || true
      # Distinguish 404 from transient network failures.
  ...
  download_collection() {
    ...
    for i in $(seq 0 299); do
      ...
      if download_one "$url" "$filepath"; then
        ...
        rc=$?
        if [ "$rc" -eq 2 ]; then
          # True 404 means there are no more shards for this collection.
          break
        fi
