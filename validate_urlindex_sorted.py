#!/usr/bin/env python3
"""
Validate that URL index parquet files are sorted by urlkey and mark them.

This script specifically handles Common Crawl URL index files, not pointer databases.
"""
...
def is_url_index_sorted(parquet_file: Path, sample_size: int = 1000) -> Tuple[bool, str]:
    ...
    try:
        ...
        # Check schema - must have 'urlkey'
        if 'urlkey' not in pf.schema.names:
            return False, f"Not a URL index file (no 'urlkey' field)"
            ...
            for rg_idx in range(1, min(pf.metadata.num_row_groups, 10)):  # Check first 10 row groups
                ...
                if len(vals) > 0:
                    ...
                    if last_val > first_val:
                        return False, f"Unsorted between row groups"
...
def check_single_file(pq_file: Path, parquet_root: Path, verify_only: bool) -> Tuple[str, Path, bool, str]:
    ...
    # Skip pointer database directories
    ...
    try:
        ...
        if is_sorted:
            ...
            if not verify_only:
                ...
                return ('sorted_unmarked', new_path, True, f'Marked as sorted')
            else:
...
def main() -> int:
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if completed % 100 == 0:
                    print(f"Progress: {completed}/{len(all_files)} - Marked: {len(already_marked)}, Newly marked: {len(sorted_unmarked)}, Unsorted: {len(unsorted_files)}", flush=True)
    ...
    if len(already_marked) + len(sorted_unmarked) + len(unsorted_files) > 0:
        pct = (len(already_marked) + len(sorted_unmarked)) / (len(already_marked) + len(sorted_unmarked) + len(unsorted_files)) * 100
