...
def _iter_parquet_files(parquet_root: Path, collections: Optional[Sequence[str]], collections_regex: Optional[str]) -> Iterator[Path]:
...
def sort_one(
    ...
    # Require enough free space to write a full-sized temp output.
    ...
    if free_bytes < (min_free_bytes + old_size):
        return False, f"skip(insufficient_free free={_format_gb(free_bytes)} need>={_format_gb(min_free_bytes + old_size)})"
    ...
    # We keep compression consistent with existing outputs (ZSTD).
    q = (
        "COPY ("
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument("--min-free-gb", type=float, default=250.0, help="Skip a file if free space is less than (min-free + file_size)")
    ...
    if args.zfs_dataset and bool(args.refuse_if_snapshots):
        # If there are snapshots, rewriting Parquet will retain the old blocks
        # and can quickly exhaust space. Force an explicit decision.
        n = _snapshot_count()
        if n:
            raise SystemExit(
