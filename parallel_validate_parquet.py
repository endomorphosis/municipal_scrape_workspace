...
Much faster than serial validation.
"""
...
def check_if_sorted(parquet_file: Path) -> Tuple[Path, bool, str]:
    """Check if parquet file is sorted. Returns (file, is_sorted, reason)."""
    try:
        con = duckdb.connect(":memory:")
        
        # Quick check: sample 100 rows from start and end
        result = con.execute(
            """
            WITH samples AS (
...
def main() -> int:
    ...
    with mp.Pool(processes=args.workers, initializer=worker_init) as pool:
        ...
        for i, (pq_file, is_sorted, reason) in enumerate(results, 1):
            ...
            if i % 100 == 0:
                print(f"Progress: {i}/{len(all_files)} - Sorted: {len(sorted_files)}, Unsorted: {len(unsorted_files)}", flush=True)
