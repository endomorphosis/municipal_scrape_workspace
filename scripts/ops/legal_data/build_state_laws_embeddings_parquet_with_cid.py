#!/usr/bin/env python3
"""Build CID-keyed embeddings parquet files for state laws.

Embeds only semantic text columns from CID-indexed source parquet:
- name
- text
"""
...
def _iter_input_parquets(input_dir: Path, recursive: bool, include_combined: bool) -> Iterable[Path]:
    ...
    for path in sorted(globber("*.parquet")):
        if path.name.endswith("_embeddings.parquet"):
            continue
        # Avoid double-counting by default: per-state + all-states in one run.
...
def run(
    *,
    ...
    manifest_path = input_dir / "embeddings_manifest.state_laws.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build CID-keyed semantic embeddings parquet files for state laws")
    ...
    parser.add_argument(
        "--include-combined",
        action="store_true",
        help="Also process *_all_states.parquet (disabled by default)",
    )
