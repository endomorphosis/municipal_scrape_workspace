#!/usr/bin/env python3
"""Build CID-keyed embeddings parquet files for state court rules."""
...
def _iter_input_parquets(input_dir: Path, recursive: bool, include_combined: bool) -> Iterable[Path]:
    ...
    for path in sorted(globber("*.parquet")):
        if path.name.endswith("_embeddings.parquet"):
            continue
        if not include_combined and path.name.endswith("_all_states.parquet"):
            continue
        if path.is_file():
            yield path
...
def run(
    *,
    ...
    for source_path in files:
        ...
        print(f"[done] {source_path.name}: source_rows={stats['source_rows']} embedded_rows={stats['embedded_rows']}")

    manifest["totals"]["unique_cids"] = len(global_cids)
    manifest_path = input_dir / "embeddings_manifest.state_court_rules.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build CID-keyed semantic embeddings parquet files for state court rules")
    ...
    parser.add_argument("--include-combined", action="store_true", help="Also process *_all_states.parquet")
