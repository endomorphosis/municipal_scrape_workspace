...
def _iter_parquet_files(parquet_root: Path, collections: Optional[Sequence[str]], collections_regex: Optional[str]) -> Iterator[Tuple[str, Path]]:
...
def _get_host_rev_col_index(meta: pq.FileMetaData) -> int:
    schema = meta.schema
    # pyarrow Parquet schema has "names" for leaf columns
