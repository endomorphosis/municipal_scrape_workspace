"""Web dashboard for ccindex ("Internet Archive-ish" local UI).

This is intentionally a thin UI wrapper around `municipal_scrape_workspace.ccindex.api`.
It is NOT required for core ccindex usage.
...
# NOTE: This file got corrupted during iterative edits (indentation + accidental
# insertion into string literals). We quarantine the broken content below so the
# module imports cleanly, then provide a fresh implementation at the end.
_CORRUPTED_DASHBOARD_PY = r'''
...
<div id='dresults' class='card' style='margin-top: 14px; display:none;'></div>
...
"""
        ...
        try:
            if tool_name == "search_domain_meta":
                ...
                out = api.fetch_warc_record_range(
                ...
                out = {
                    ...
                    "status": out.status,
                    "url": out.url,
                    "bytes_requested": out.bytes_requested,
                    "bytes_returned": out.bytes_returned,
                    "sha256": out.sha256,
                    "decoded_text_preview": out.decoded_text_preview,
                    "error": out.error,
                }
            ...
            # Return the tool result directly; dashboard client expects plain JSON.
...
<div class='card'>
  ...
  <div class='small' style='margin-top: 10px;'>
    Uses master meta-index: <span class='code'>{html.escape(str(master_db))}</span>
        • MCP JSON-RPC: <span class='code'>POST /mcp</span> • SDK: <a class='code' href='/static/ccindex-mcp-sdk.js'>ccindex-mcp-sdk.js</a>
  </div>
</div>
"""
        ...
        const rows = (records || []).map((r, idx) => {{
            ...
            return `
                <tr>
                    ...
                    <td><a href='${{esc(r.url||"#")}}' target='_blank' rel='noreferrer'>${{url}}</a><div class='small'>${{ts}}</div></td>
    ...
    async function run() {{
        try {{
            ...
            statusEl.innerHTML = `
                ...
                <div class='small'>download_url: <a class='code' href='${{esc(res.url)}}' target='_blank' rel='noreferrer'>${{esc(res.url)}}</a></div>
            `;

            previewEl.textContent = res.decoded_text_preview || '';
...
"""
    ...
    # Compatibility JSON endpoints remain via MCP JSON-RPC at /mcp.
